...
The pols-month dataset (renamed "politicians_data") contains data about the numerical breakdown of republicans/democrats in Congress for each month from `r min(pull(politicians_data, year))` to `r max(pull(politicians_data, year))`. There are `r nrow(politicians_data)` observations in this dataset, and `r ncol(politicians_data)` columns.

The snp dataset (snp_data) contains the closing values of the S&P stock index for every month from `r min(pull(snp_data, year))` to `r max(pull(snp_data, year))`.

The unemployment dataset (unemployment) contains the US unemployment rate of every month of every year since 1948. 
